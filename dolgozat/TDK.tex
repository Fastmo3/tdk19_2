...
\item belső helyek esetén: $\sum_{y(ki)} I_y\leq \sum_{y(be)}$
\item tranziciók esetén: $\sum_y I_y\leq C_x$ ($y:$ bejövő élek)
\item $\sum_{y(ki)} I_y = \sum_{y(be)}$
...
Az egyes belső helyeken a bufferbe áramló tokenek  eredő intentitása:

$$F= \sum$$
TODO!!
%ezt hogyan????
%F = 	Szumma_{x_belső hely} 
%( Szumma_{y:bejövő élek x-nél} - Szumma_{y:kimenő élek  x-nél} )
